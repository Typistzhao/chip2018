...
train_r = pd.read_csv('../input/train_remix.csv')
...
if not os.path.exists("../input/word_embed.own%s" % seed):
    corpus = np.concatenate([train.words_x.values, train.words_y.values, test.words_x.values, test.words_y.values],
                            axis=0)
    seg_corpus = [_.split() for _ in corpus]
    print(len(seg_corpus))
    word2vec = Word2Vec(seg_corpus, size=200, min_count=2, sg=1, hs=0, negative=10, iter=10, workers=42, window=5,
                        seed=seed)
...
if not os.path.exists("../input/char_embed.own%s" % seed):
    ...
    corpus = np.concatenate([train.chars_x.values, train.chars_y.values, test.chars_x.values, test.chars_y.values],
                            axis=0)
    seg_corpus = [_.split() for _ in corpus]
    print(len(seg_corpus))
    word2vec = Word2Vec(seg_corpus, size=200, min_count=2, sg=1, hs=0, negative=10, iter=10, workers=40, window=6,
                        seed=seed)
...
if not os.path.exists("../cache/features_aug_%s.jl" % seed):
    ...
    cols = ["min_kcore", "max_kcore", "common_neighbor_count", "common_neighbor_ratio", "min_freq", "max_freq", 'qid1',
            'qid2']
    ...
    train_r_decom_diff = pd.DataFrame(np.abs(train_r_q1_decom - train_r_q2_decom),
                                      columns=["decom_diff_%s" % i for i in range(4)])
    train_r_decom_angle = pd.DataFrame(train_r_q1_decom * train_r_q2_decom,
                                       columns=["decom_angle_%s" % i for i in range(4)])

    test_r_decom_diff = pd.DataFrame(np.abs(test_r_q1_decom - test_r_q2_decom),
                                     columns=["decom_diff_%s" % i for i in range(4)])
    test_r_decom_angle = pd.DataFrame(test_r_q1_decom * test_r_q2_decom,
                                      columns=["decom_angle_%s" % i for i in range(4)])
    ...
    def extract_hash_subgraph_features(df):
        ...
        hash_subgraph_features = Parallel(n_jobs=40)(
            delayed(hash_subgraph)(x[0], x[1]) for x in df[['q1', 'q2']].values)
    ...
    features_hash_subgraph_train_r = train_r[
        ["hash_subgraph_qmax", "hash_subgraph_qmin", "hash_subgraph_qdiff", "hash_subgraph_qmean"]]
    features_hash_subgraph_test_r = test_r[
        ["hash_subgraph_qmax", "hash_subgraph_qmin", "hash_subgraph_qdiff", "hash_subgraph_qmean"]]
    ...
    features_test = pd.concat(
        ...
        axis=1).values
    features_train_r = pd.concat(
        [features_magic_train_r, train_r_decom_angle, train_r_decom_diff, features_hash_subgraph_train_r,
         pagerank_features_train_r], axis=1).values
    features_test_r = pd.concat(
        [features_magic_test_r, test_r_decom_angle, test_r_decom_diff, features_hash_subgraph_test_r,
         pagerank_features_test_r], axis=1).values
...
for train_idx, val_idx in StratifiedKFold(n_splits=10, shuffle=True, random_state=42).split(y, y):
    ...
    sample_weight = np.concatenate(
        [np.array([1.0] * len(train_y)), np.array([0.20] * len(y_r[remix_index[model_count]]))], axis=0)
    ...
    best_model_path = "../models/" + "lstm_cross_char_add_feature_augmentation_best_model_finetune" + str(
        model_count) + ".h5"
    model_checkpoint = ModelCheckpoint(best_model_path, save_best_only=True, save_weights_only=True)
    if not os.path.exists(best_model_path):
        hist = model.fit([train_q1_char_seq_2, train_q2_char_seq_2, features_train_2],
    ...
    test['y_pre_1_%s' % model_count] = model.predict([test_q1_char_seq, test_q2_char_seq, features_test],
                                                     batch_size=2 ** 9)
    test[['y_pre_1_%s' % model_count]].rename(columns={'y_pre_%s' % model_count: "y_pre"}).to_csv(
        "../output/lstm_cross_char_add_feature_augmentation_finetune_%s.csv" % model_count)
...
submit[['y_pre']].to_csv('../output/lstm_cross_char_add_feature_augmentation_finetune_cv10_mean.csv', index=False,
                         float_format='%.10f')
...
submit['y_pre'] = submit[pred_cols].mean(axis=1)
submit.to_pickle('../output/lstm_cross_char_add_feature_augmentation_finetune_10cv_mean.pickle')
submit[['y_pre']].to_csv('../output/lstm_cross_char_add_feature_augmentation_finetune_cv10_mean_r.csv', index=False,
                         float_format='%.10f')